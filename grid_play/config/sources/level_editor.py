...
def generate_code_from_palette(
    ...
    *,
    ...
    a("from grid_universe.grid.gridstate import GridState")
    a("from grid_universe.grid.convert import to_state")
    a("from grid_universe.movements import MOVEMENT_REGISTRY")
    a("from grid_universe.objectives import OBJECTIVE_REGISTRY")
    a(env_import_line)
    for imp in dedup_imports:
        a(imp)
    a("")
    a("def build_level() -> GridState:")
    tl_arg = f", turn_limit={int(cfg.turn_limit)}" if cfg.turn_limit is not None else ""
    a(
        f"    gridstate = GridState(width={cfg.width}, height={cfg.height}, "
        f"movement=MOVEMENT_REGISTRY[{repr(move_key)}], "
        f"objective=OBJECTIVE_REGISTRY[{repr(obj_key)}], "
